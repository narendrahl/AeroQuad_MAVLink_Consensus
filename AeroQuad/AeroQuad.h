...
#define SOFTWARE_VERSION 3.2
  ...
  #define BAUD CONFIG_BAUDRATE
    ...
    #define BAUD 57600 // use this to be compatible with USB and XBee connections
  #else
    #define BAUD 57600
  #endif
#endif  
...
float At = 0.6 , rollVel , pitchVel , RSD , PSD , Cx , Cy ;
long Rx , Py ;
int NN = 2;
long p , q , p5 ,q5;
float G_Dt = 0.002; 
...
// Sudeep GPS count variable
unsigned long int countSudeep = 0;
// Sudeep Altitude Hold Correction variable
int altitudeHoldCorrectionSudeep = 0;
float Manual_Altitude = 2.6;
//////////////////////////////////////////////////////
 ...
 */
 ...
 // special state that allows immediate turn off of Altitude hold if large throttle changesa are made at the TX
  ...
  int minThrottleAdjust = -35;
  int maxThrottleAdjust = 45;
  int altitudeHoldThrottle = 1550;
  int previousaltitudeHoldState = 0; //Soniya 
  int previousPositionHoldState = 0; //Sudeep 
...
#define MAX_WAYPOINTS 16  // needed for EEPROM adr offset declarations
  ...
  #define DEFAULT_HOME_ALTITUDE 3  // default home base altitude is equal to 5 meter
  ...
  #if defined LeaderFollower
    GeodeticPosition waypointLF =GPS_INVALID_POSITION, readWaypointLF=GPS_INVALID_POSITION;  
