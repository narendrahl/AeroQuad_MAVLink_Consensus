...
float S_P_X = 0.0 , S_P_Y = 0.0;
int j = 0;
boolean Calibrated_XY = false ;
...
void calculateFlightError()
{
 ...
 /*   if(j>50)
    ...
    Serial2.print(":");
    Serial2.print(rollAttitudeCmd);
    Serial2.print(":");
    Serial2.print(motorAxisCommandRoll);
    Serial2.print(":");
    Serial2.print(pitchAttitudeCmd);
    Serial2.print(":");
    Serial2.print(motorAxisCommandPitch);
    Serial2.print(":");
    Serial2.print(receiverCommand[XAXIS]);
    Serial2.print(":");
...
//  motorAxisCommandRoll +=  updatePID(S_P_X , kinematicsAngle[XAXIS] , &PID[RATE_XAXIS_PID_IDX]);  
//  motorAxisCommandPitch += updatePID(-(S_P_Y) , -kinematicsAngle[YAXIS], &PID[RATE_YAXIS_PID_IDX]);
    }
    else
  #endif
...
/*    
    motorAxisCommandRoll   = updatePID(0, gyroRate[XAXIS], &PID[ATTITUDE_GYRO_XAXIS_PID_IDX]);
    motorAxisCommandPitch  = updatePID(0, -gyroRate[YAXIS], &PID[ATTITUDE_GYRO_YAXIS_PID_IDX]);
*/

/*
Serial2.print("X");
Serial2.print(kinematicsAngle[XAXIS]);
Serial2.print(":");
Serial2.print(-kinematicsAngle[YAXIS]);
Serial2.print(":");
*/
 ...
 * 
 * This function will add some throttle input if the craft is angled
...
void processThrottleCorrection() {
  ...
  if (inFlight && flightMode == ATTITUDE_FLIGHT_MODE) 
  {
  throttleAdjust = throttle / (cos (kinematicsAngle[XAXIS]*0.55) * cos (kinematicsAngle[YAXIS]*0.55));  // Correction factor added next on 11-05
  throttleAdjust = constrain ((throttleAdjust - throttle), 0, 150); //compensate max  +/- 25 deg XAXIS or YAXIS or  +/- 18 ( 18(XAXIS) + 18(YAXIS))
  }
  ...
  //sudeep
  throttle = constrain((throttle + throttleAdjust),MINCOMMAND,MAXCOMMAND-50);  // limmit throttle to leave some space for motor correction in max throttle manuever
}
...
void processFlightControl() {
  ...
  // If throttle in minimum position, don't apply yaw //sudeep
  if (receiverCommand[THROTTLE] < MINCHECKthrottle) {
    for (byte motor = 0; motor < LASTMOTOR; motor++) {
      motorMinCommand[motor] = minArmedThrottle;
