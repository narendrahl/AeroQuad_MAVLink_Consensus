/*
  ...
  An Open Source Arduino based multicopter.NATIVE_BIG_ENDIAN
...
void sendSerialGpsPostion() {
    ...
    if (haveAGpsLock())
    {
      #if defined(AltitudeHoldBaro)
       // ii = ii + 1;
       mavlink_msg_global_position_int_pack(MAV_SYSTEM_ID, MAV_COMPONENT_ID, &msg, millisecondsSinceBoot, currentPosition.latitude, currentPosition.longitude, 0, 0, 0, 0, 0, ((int)(trueNorthHeading / M_PI * 180.0) + 360) % 360);
//        mavlink_msg_global_position_int_pack(MAV_SYSTEM_ID, MAV_COMPONENT_ID, &msg, millisecondsSinceBoot, currentPosition.latitude, currentPosition.longitude, curr_X, curr_Y , RSD, PSD, 0, ((int)(trueNorthHeading / M_PI * 180.0) + 360) % 360);
      #else
...
void readSerialCommand() {
  ...
  Serial_Available_Count=0;
     ...
     if(Serial_Available_Count=SERIAL_PORT.available()) {
       
   while(Serial_Available_Count>0)
   {
   ...
   if(msg.sysid == 123)
   {
    if(msg.msgid == 33)
      {
          p  = mavlink_msg_global_position_int_get_lat(&msg);
          q  = mavlink_msg_global_position_int_get_lon(&msg);
//          Cx = mavlink_msg_global_position_int_get_alt(&msg);
//          Cy = mavlink_msg_global_position_int_get_relative_alt(&msg);
//          Rx = mavlink_msg_global_position_int_get_vy(&msg);  
//          Py = mavlink_msg_global_position_int_get_vz(&msg);
         
         readWaypointLF.latitude = p ;
         readWaypointLF.longitude = q ;

         waypointLF.latitude= readWaypointLF.latitude;
         waypointLF.longitude= readWaypointLF.longitude;
...
//          Py = mavlink_msg_global_position_int_get_vz(&msg);
   ...
   if(msg.sysid == 122)
   {
    if(msg.msgid == 33)
      {
          p5  = mavlink_msg_global_position_int_get_lat(&msg);
          q5  = mavlink_msg_global_position_int_get_lon(&msg);
//          Cx = mavlink_msg_global_position_int_get_alt(&msg);
//          Cy = mavlink_msg_global_position_int_get_relative_alt(&msg);
//          Rx = mavlink_msg_global_position_int_get_vy(&msg);  
//          Py = mavlink_msg_global_position_int_get_vz(&msg);
         
         readWaypointLF.latitude = p5 ;
         readWaypointLF.longitude = q5 ;

         waypointLF.latitude= (p+p5)/2;
         waypointLF.longitude= (q+q5)/2;
