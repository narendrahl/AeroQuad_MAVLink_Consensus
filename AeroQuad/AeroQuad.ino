...
#ifdef ArduCopter
  #define LED_Green 25//green is blue in APM
  #define LED_Red 27
...
void setup() {
  ...
  // Sudeep GPS testing declaration (comment it)
  //navigationState = OFF;
}
...
void process50HzTask() {
  ...
  #ifdef AltitudeHoldRangeFinder
  /*  digitalWrite(rangeFinders[0].triggerpin, LOW);
    delay(0.5);
    digitalWrite(rangeFinders[0].triggerpin, HIGH);
    delay(0.5);
    digitalWrite(rangeFinders[0].triggerpin, LOW);
    pinMode(rangeFinders[0].pin, INPUT); */
    updateRangeFinders();
  //  Serial.println(pulseIn((rangeFinders[0].pin),HIGH));
  #endif
...
Serial.flush();
#ifdef Follower
    if (navigationState == 11)
    {
    Serial.print(MAV_SYSTEM_ID);
    Serial.print(':');
    
//    Serial.print(MAX_NAVIGATION_ANGLE_CORRECTION);
//    Serial.print(':');
    
//    Serial.print(baroAltitudeToHoldTarget);
 //   Serial.print(':');
//    Serial.print(curr_alt);
 //   Serial.print(':');
//    Serial.print(getBaroAltitude());
//    Serial.print(':');
//    Serial.print(distanceToDestination);
//    Serial.print(':');
    Serial.print(distanceToDestinationX);
    Serial.print(':');
    Serial.print(distanceToDestinationY);
    Serial.print(':');
//    Serial.print(motorAxisCommandRoll);
//    Serial.print(':');
//    Serial.print(motorAxisCommandPitch);
//    Serial.print(':');
  //  Serial.print(motorAxisCommandYaw);
//    Serial.print(':');
//    Serial.print(throttle);
//    Serial.print(':');
    Serial.print(degrees(angleToWaypoint));
//    Serial.print(':');
//    Serial.print(positionHoldPointToReach.latitude);
 // Serial.print(':');
 // Serial.print(positionHoldPointToReach.longitude);
    Serial.print(':');
    Serial.print(missionPositionToReach.latitude);
    Serial.print(':');
    Serial.print(missionPositionToReach.longitude);
 //   Serial.print(':');
 //   Serial.print(waypointLF.latitude);
  //  Serial.print(':');
 //   Serial.print(waypointLF.longitude);
 //   Serial.print(':');
//    Serial.print(estimatedPosition.latitude);
 //   Serial.print(':');
 //   Serial.print(currentPosition.longitude);
    Serial.print(':');
    Serial.print(degrees(trueNorthHeading));
    Serial.print(':');
    ...
    Serial.print(':');
    Serial.print(gpsPitchAxisCorrection);
    Serial.print(':');
    Serial.print(gpsYawAxisCorrection);
    Serial.print(':');
//    Serial.print(currentSpeedRoll);
//   Serial.print(':');
//    Serial.print(navigationState);
//    Serial.println(':');
//    Serial.print(currentSpeedPitch);
 //   Serial.println(':');
    }
...
if((CalibrateOnce == 1) && (receiverCommand[4] < 1100))  //// Rate Mode Switch for Calibration
{

delay(500); // 0.5 second , 2 readings in 1 second
Xsum += getMagnetometerRawData(0); // XAXIS
Ysum += getMagnetometerRawData(1);
Zsum += getMagnetometerRawData(2);
data_count ++ ;

if ( receiverCommand[4] > 1100)

     {
  
  CalibrateOnce = 0 ;
  magBias[0]  = 0 - (Xsum/data_count);
  magBias[1]  = 0 - (Ysum/data_count);
  magBias[2]  = 0 - (Zsum/data_count);
